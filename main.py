...
def try_groq_request(messages: List[Dict]) -> str:
    """Attempt to generate response with Groq model"""
    try:
        ...
        print(f"Failed to use Groq: {str(e)}")
        return None
...
def try_llm_request(client, model_name: str, messages: List[Dict]) -> str:
    ...
    try:
        ...
        groq_response = try_groq_request(messages)
        if groq_response:
...
def enhance_search_query_with_llm(user_query: str) -> str:
    ...
    messages = [
        {"role": "system", "content": "You are a stock market expert. Turn the query into a focused description of the business model, industry, and key traits like growth, financials, or trends to find relevant stocks."},
...
def generate_analysis_with_llm(stocks_data: List[dict]) -> str:
    ...
    messages = [
        {"role": "system", "content": "You are a stock market analyst. Compare the stocks' performance metrics and business models, highlighting key differences, opportunities, and risks."},
...
def add_market_trend_radar(all_stock_data):
    ...
    colors = {
    0: 'rgb(0, 255, 255)',      # Cyan
    1: 'rgb(255, 20, 147)',     # Deep Pink
    2: 'rgb(75, 0, 130)',       # Indigo
    3: 'rgb(255, 215, 0)',      # Gold
    4: 'rgb(60, 179, 113)',     # Medium Sea Green
    5: 'rgb(138, 43, 226)'      # Blue Violet
}
...
def main():
    ...
    st.write("Enter the stock to generate intelligent market data for strategic investment:")
